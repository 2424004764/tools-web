...
export class ResumeRouter {
  constructor(db, env = null, waitUntil = null) {
    this.controller = new ResumeController(db, env, waitUntil)
  }
  ...
  async handle(request, path, env, origin) {
    ...
    switch (request.method) {
      case 'GET':
        if (!hasId) {
          // GET /api/resumes - 获取当前用户的所有简历（支持分页）
          const pager = Pager.fromRequest(request)
          return await this.controller.index(user, pager, origin)
        } else {
          // GET /api/resumes/{id} - 根据ID获取当前用户的简历
          return await this.controller.show(id, user, origin)
        }

      case 'POST':
        // POST /api/resumes - 为当前用户创建简历
        if (hasId) {
          return ApiResponse.error('创建简历不需要提供ID', origin, 400)
        }
        const createData = await request.json()
        return await this.controller.store(createData, user, origin)

      case 'PUT':
        // PUT /api/resumes/{id} - 更新当前用户的简历
        if (!hasId) {
          return ApiResponse.error('更新简历需要提供ID', origin, 400)
        }
        const updateData = await request.json()
        return await this.controller.update(id, updateData, user, origin)

      case 'DELETE':
        // DELETE /api/resumes/{id} - 删除当前用户的简历
        if (!hasId) {
          return ApiResponse.error('删除简历需要提供ID', origin, 400)
        }
