...
import { CompanyWebsitesRouter } from '../routes/companyWebsites.js'

export async function onRequest(context) {
  const { request, env } = context
  const url = new URL(request.url)
  const path = url.pathname.replace('/api/company-websites', '')
  ...
  try {
    // 创建路由实例并处理请求
    const router = new CompanyWebsitesRouter(dbInit.db)
    return await router.handle(request, path, env, origin)
  } catch (error) {
    console.error('CompanyWebsites API error:', error)
