// 音乐播放列表 —— 登录用户的私有 CRUD
//   POST   /api/music-playlist/songs/upload-url          签发 R2 SigV4 预签名 PUT URL（预扣统一存储额度）
//   POST   /api/music-playlist/songs                     创建歌曲元数据（按 R2 真实大小结算额度）
...
//   DELETE /api/music-playlist/songs/{id}                删除（含 R2 对象 + 级联 join + 退存储额度）
...
//
// 存储计费：并入统一存储额度（079 迁移），原 /songs/quote 与 /songs/{txId}/reverse
// 积分计费端点已随 079 下线；上传失败的预留走 POST /api/storage-quota/release 释放，
// 或等 1 小时自动失效。
//
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (error instanceof ValidationError) {
      // 存储额度不足 → 402，前端据此引导购买
      const isQuotaError = typeof error.message === 'string' && error.message.includes('存储空间不足')
      return ApiResponse.error(error.message, origin, isQuotaError ? 402 : 400)
    }
