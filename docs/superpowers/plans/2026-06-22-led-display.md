...
在 LED 屏幕和配置面板之间插入 H5 专属的「展开配置」按钮（仅移动端可见）。同时把现有的「显示/隐藏配置面板」PC 端按钮留在原处不动。
...
PC 上的「显示/隐藏」按钮（在 Task 3 之前 Task 1 引入的）继续保留，H5 不显示即可（用 `hidden md:block` 类）。如果之前 Task 1 引入的按钮还在，把它包成：
...
最终 `<div v-if="showPanel">` 内容就是 Task 3 写的配置表单，**不要**带 `md:block hidden` 类，纯靠 `v-if="showPanel"` 控制显隐。
