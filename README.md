...
## 功能开发日志
- 2026-08-25 新增音乐播放列表工具：登录后上传 MP3 / M4A / WAV 音频至 Cloudflare R2（浏览器直传、预签名 URL 15 分钟有效），歌曲与歌单相互独立、可任意归属多份；每首歌与每个歌单都有独立公开分享链接（8 位随机短码），无需登录即可播放，播放次数每次点击 +1
- 2026-08-24 新增物品比价工具：登录后记录同一商品在淘宝、京东、拼多多、1688、官网、线下等不同平台的售价与最终实付价（单价×数量+运费-优惠自动计算），自动 👑 高亮最低价、一键标记最终购买平台（同步把物品标记为已购买）、每个平台独立备注/链接/卖家/购买日期/状态、累计花费 + 比价节省金额 + 分类汇总 + 价差 TOP5 + JSON 导出
- 2026-08-23 新增在线涂鸦画板工具：纯前端 HTML5 Canvas，支持画笔/橡皮切换、10 种预设颜色 + 自定义拾色器、可调画笔粗细（1~60px）与橡皮大小（4~80px）、撤销/重做最多 30 步历史、画布尺寸与背景色可设置（含透明背景）、内置 5 种常用尺寸预设（640×480 ~ 1920×1080）、一键下载 PNG 或复制到剪贴板，支持鼠标与触摸操作
- 2026-08-23 新增高亮代码生成图片工具：左侧编辑代码、选择 190+ 语言与 8 套高亮主题（GitHub/Night/Atom One Dark/Monokai 等），右侧实时预览；可调字体、字号、行高、内边距、圆角、窗口控件、行号与水印；一键 html2canvas 截图下载 PNG / 复制到剪贴板 / 复制高亮 HTML，纯前端本地处理保护隐私
- 2026-08-23 新增在线计算器：集成科学计算（三角函数/对数/指数/阶乘/幂/常量/DEG-RAD 角度切换/历史记录/MC-MR-M+/M- 记忆）与 2/8/10/16 进制互转，Tab 切换两种模式，纯前端运算，键盘可直接输入
- 2026-08-21 新增每月固定开销工具：单用户记录房租/订阅/贷款/保险等周期性开销，支持分类饼图汇总、月度+年度金额、扣款日提醒、启用/停用切换、起止日期（适合分期）、分享与导出
- 2026-08-21 优化图片拼图工具：上传区与画布统一强制 1:1 正方形，cell 按 gridCols/gridRows 拉伸，去掉间隙调整 UI，缩略图与预览保持一致；上传后文件名带时间戳避免重复
- 2026-08-21 图片拼图工具支持拖动图片调整显示位置（鼠标悬停按钮：重置位置 / 替换 / 删除）
- 2026-08-21 图片拼图工具新增拼贴卡片模板：错位叠加、旋转角度、拍立得白边、阴影等效果
- 2026-08-21 图片拼图工具新增拼贴/网格分类 tab 切换，拼贴分类下含杂志风、L 形、三角堆叠、海报、X 形等 13 种模板
- 2026-08-21 图片拼图工具槽位操作面板独立于画布，避免叠加卡片遮挡 hover 按钮；下载按钮移至画布上方
- 2026-08-21 图片拼图工具下载图与画布位置一致（修复 canvas 旋转中心点计算 bug）
- 2026-08-20 图片拼图工具新增 13/14/15 槽位模板，按槽位数分组显示与 push 数严格一致
- 2026-08-19 新增图片拼图工具：支持宫格（四/九/十六宫格）、横版/竖版长条、阶梯递增/递减、电影海报、大图居中、心形/圆形异形遮罩等多种拼图模板，一键合成多图排版，可下载合并图或打包 ZIP
- 2026-07-19 新增 24 点（算 24）游戏：系统随机发 4 张牌，玩家用 +、-、×、÷ 和括号算出 24，内置求解器可一键提示，包含三档难度、连击、本局历史与实时表达式求值
- 2026-07-18 新增点灯小游戏（Lights Out）：5×5 灯泡棋盘，点击一盏灯切换自身和上下左右四盏邻居灯的状态，挑战把所有灯都熄灭，支持多种难度、撤销、提示与最佳纪录
...
- 高亮代码生成图片（190+ 语言、8 套主题、可自定义样式与水印）
